...
class TextExtractor:
    def __init__(self, 
        path: str,
        toc_keywords: List = None,
        keywords_to_ignore: List = None
        ):
        self.path = path
        if not toc_keywords:
        ...
        self.service_dir = path.split("/")[-1].split(".pdf")[0].lower().replace("/", "_").replace("\\", "_")
    ...
    def find_toc(self, pdf) -> int:
        for page_num, page in enumerate(pdf.pages[:10]):  # Limit the search to the first X pages
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            for i, page in enumerate(pdf.pages):
                ...
                if text and any(keyword in text for keyword in self.toc_keywords):
                    index_toc.append(i + 1)  # Le pagine in pdfplumber sono 0-indexed, mentre per l'utente sono 1-indexed
                    ...
                    for line in list_lines:
                        ...
                        if match:
                            ...
                            toc.append({"number": main_chapters[0], "title": main_chapters[1].strip(), "page": main_chapters[2]})
                ...
                for i, page in enumerate(pdf.pages):
                    ...
                    if index_toc[0] + 1 == i:
                        ...
                        for next_line in next_page_lines:
                            ...
                            next_match = re.match(r"(\d+(?:\.\d+)?)\s+(.+?)\s+(\d+)$", next_line)
                            if next_match:
                                next_chapters = next_match.groups()                       
                                toc.append({"number": next_chapters[0], "title": next_chapters[1].strip(), "page": next_chapters[2]})
    ...
    def extract_full_text(self):
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            for i, page in enumerate(pdf.pages):
                ...
                with open(f"extracted_pages/{self.service_dir}/page_{i}.txt", "w", encoding="utf-8") as file:
    ...
    def extract_keywords(self, pages: List[int]) -> dict:
        # Inizializza bold_text
        bold_text = {}
        # Significant colors for special text
        # significant_colors = [
        #     (1, 0, 0),   # Red
        #     (0, 0, 1),   # Blue
        #     (0, 1, 0),   # Green
        # ]
        # Estrai testo in grassetto dalle pagine PDF
        with pdfplumber.open(self.path) as pdf:
            ...
            for page in pdf.pages:
                if page.page_number not in pages: # Se la pagina non è una pagina di indice 
                    page_chars = []
                    for char in page.chars:
                        if ((
                            'Bold' in char.get("fontname", "") or 
                            # char.get("stroking_color", None) in significant_colors or
                            char.get("stroking_color", None) != (0,)) or 
                            char.get('size', 0) > size_threshold
                                 ):
        ...
        for key, value in bold_text.items():
            ...
            for v in values_list:
                ...
                is_ignored = any(keywords_to_ignore in v for keywords_to_ignore in self.keywords_to_ignore)
...
def clean_text(text: str) -> str:
    text = text.strip().replace(":", "").replace(",", "").replace(".", "").replace(";","").replace("’", "").replace("‘", "").replace("(", "").replace(")", "")
    return text

def clean_keywords(keywords: Dict[int, List[str]]) -> Dict[int, List[str]]:
    for key, value in keywords.items():
        for keyword in value:
            if keyword == "" or keyword == " " or len(keyword) < 2 or keyword.isdigit() or len(keyword) > 15:
